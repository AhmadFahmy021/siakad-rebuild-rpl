...
@section('main')
    ...
    <div class="row col-12 ">
        <div class="card">
            <h4 class="card-header">Edit Siswa</h4>
            <div class="card-body">
                <form action="{{ url('admin/kelola/siswa/' . $siswa->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group mb-3">
                        <label for="name" class="form-label">Name</label>
                        <input class="form-control" id="name" disabled type="text" value="{{ $siswa->user->name }}" readonly>
                    </div>
                    <div class="form-group mb-3">
                        <label for="email" class="form-label">Email</label>
                        <input class="form-control" id="email" name="email" disabled type="email" value="{{ $siswa->user->email }}" readonly>
                    </div>
                    <div class="form-group mb-3">
                        <label for="username" class="form-label">Username</label>
                        <input class="form-control" id="username" name="username" disabled type="text" value="{{ $siswa->user->username }}" readonly>
                    </div>
                    <div class="form-group mb-3">
                        <label for="orang_tua" class="form-label">Orang Tua</label>
                        <input class="form-control @error('orang_tua')
                            is-invalid
                        @enderror" id="orang_tua" name="orang_tua" type="text" value="{{ $siswa->nama_ortu }}">
                        @error('orang_tua')
