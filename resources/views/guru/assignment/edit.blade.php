...
@section('main')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box py-3">
                ...
                <h2 class="text-dark fw-bold m-0 font-22">Edit Assignment</h2>
    ...
    <!-- Main Edit Form -->
    <form action="{{ route('assignment.update', $tugas->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="row mb-3">
            <div class="col-12 d-flex justify-content-end gap-2">
                ...
                <button type="submit" name="status" value="PUBLISHED" class="btn btn-warning font-13 px-4">Edit / Update</button>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                ...
                <div class="card shadow-sm border border-light mb-3">
                    ...
                    <div class="card-body pt-2">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control border @error('title') is-invalid @enderror" placeholder="e.g., Weekly Calculus Worksheet - Week 4" value="{{ old('title', $tugas->title) }}" required>
                            @error('title')
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="matapelajaran_id" class="form-select border @error('matapelajaran_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($subjects as $subj)
                                        <option value="{{ $subj->id }}" {{ old('matapelajaran_id', $tugas->matapelajaran_id) == $subj->id ? 'selected' : '' }}>{{ $subj->nama }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="tipe" class="form-select border @error('tipe') is-invalid @enderror" required>
                                    <option value="Homework" {{ old('tipe', $tugas->tipe) == 'Homework' ? 'selected' : '' }}>Homework</option>
                                    <option value="Quiz" {{ old('tipe', $tugas->tipe) == 'Quiz' ? 'selected' : '' }}>Quiz</option>
                                    <option value="Practical" {{ old('tipe', $tugas->tipe) == 'Practical' ? 'selected' : '' }}>Practical (Lab)</option>
                                    <option value="Writing" {{ old('tipe', $tugas->tipe) == 'Writing' ? 'selected' : '' }}>Writing / Essay</option>
                                    <option value="Attendance" {{ old('tipe', $tugas->tipe) == 'Attendance' ? 'selected' : '' }}>Attendance</option>
                                    <option value="Project" {{ old('tipe', $tugas->tipe) == 'Project' ? 'selected' : '' }}>Project</option>
                                </select>
                                @error('tipe')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-2">
                            ...
                            <textarea name="description" rows="6" class="form-control border @error('description') is-invalid @enderror" placeholder="Provide detailed instructions for the students. You can include links or formatting requirements." required>{{ old('description', $tugas->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="card shadow-sm border border-light mb-3">
                    ...
                    <div class="card-body pt-2">
                        ...
                        @if($tugas->file_path)
                            <div class="alert alert-info d-flex align-items-center justify-content-between py-2 border-0" style="background-color: rgba(91, 109, 240, 0.08); color: #3b4cb8; font-size: 13px;">
                                <div>
                                    <i class="fe-file-text me-1 font-16"></i> Current Attachment: 
                                    <a href="{{ asset('storage/' . $tugas->file_path) }}" target="_blank" class="fw-semibold text-decoration-underline text-blue">{{ basename($tugas->file_path) }}</a>
                                </div>
                                <span class="text-muted font-11">Uploaded</span>
                            </div>
                        @endif
                        ...
                        <div class="border rounded p-4 text-center cursor-pointer position-relative mb-3 bg-light bg-opacity-25" style="border: 2px dashed #d5dbdf !important; border-radius: 6px !important;">
                            ...
                            <h5 class="text-dark fw-bold m-0 font-13" id="upload-box-title">Click or Drag to Upload New File</h5>
                            <p class="text-muted font-11 mt-1 mb-2" id="upload-box-subtitle">PDF, DOCX, JPG or ZIP files (Max 25MB per file - Leave empty to keep current)</p>
                        ...
                        <div>
                            ...
                            <div class="input-group">
                                ...
                                <input type="url" name="link" class="form-control border @error('link') is-invalid @enderror" placeholder="https://example.com/materials" value="{{ old('link', $tugas->link) }}">
                            </div>
                            @error('link')
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body pt-2">
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="max_score" class="form-control border" value="{{ old('max_score', $tugas->max_score) }}" required min="0" max="1000">
                            </div>
                            @error('max_score')
                        ...
                        <div>
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="kkm" class="form-control border" value="{{ old('kkm', $tugas->kkm) }}" required min="0" max="1000">
                            </div>
                            @error('kkm')
                                <div class="text-danger font-12 mt-1">{{ $message }}</div>
                            @enderror
                ...
                <!-- Class & Deadlines Card -->
                <div class="card shadow-sm border border-light mb-3">
                    <div class="card-header bg-white border-bottom-0 pt-3 pb-0">
                        <h4 class="header-title text-dark fw-bold m-0 font-14">
                            <i class="mdi mdi-clock-outline me-1 text-blue"></i> Class & Deadlines
                        </h4>
                    </div>
                    <div class="card-body pt-2">
                        <!-- Single Target Class Selection -->
                        <div class="mb-3">
                            <label class="form-label font-13 text-dark fw-semibold">Class</label>
                            <select name="kelas_id" class="form-select border @error('kelas_id') is-invalid @enderror" required>
                                @foreach($classes as $cls)
                                    <option value="{{ $cls->id }}" {{ old('kelas_id', $tugas->kelas_id) == $cls->id ? 'selected' : '' }}>Kelas {{ $cls->name }}</option>
                                @endforeach
                            </select>
                            @error('kelas_id')
                                <div class="text-danger font-12 mt-1">{{ $message }}</div>
                        ...
                        <div>
                            ...
                            <div class="input-group">
                                ...
                                <input type="datetime-local" name="due_date" class="form-control border" value="{{ old('due_date', \Carbon\Carbon::parse($tugas->due_date)->format('Y-m-d\TH:i')) }}" required>
                            </div>
                            @error('due_date')
                                <div class="text-danger font-12 mt-1">{{ $message }}</div>
                            @enderror
    ...
    @section('js')
        <script>
            document.getElementById('file_tugas').addEventListener('change', function(e) {
                var fileName = e.target.files[0] ? e.target.files[0].name : "Click or Drag to Upload New File";
